...
```{r}
coastal_sally_clean <- and_vertebrates |>
  filter(species == "Coastal giant salamander") |>
  drop_na(unittype, section)

cont_table_coastal_sally <- table(coastal_sally_clean$section, coastal_sally_clean$unittype)

chisq.test(cont_table_coastal_sally)
```
The extremely small p-value that we get indicates that there is a significant relationship between forest type and channel unit for coastal giant salamander abundance. Thus, we reject the null hypothesis that forest type and channel unit are independent. 
...
```{r}
coastal_sally_clean <- coastal_sally_clean |>
  drop_na(weight_g)

cc_biomass <- coastal_sally_clean |>
  filter(section == "CC") |>
  pull(weight_g)

og_biomass <- coastal_sally_clean |>
  filter(section == "OG") |>
  pull(weight_g)

var.test(cc_biomass, og_biomass)
```
Since the results of the test show a low p-value that is < 0.05, we are able to reject the null hypothesis. Meaning, the variances in biomass between clear cut and old growth forests are not equal. 
However because the t-test assumes a normal distribution and the data has unequal variances I will now use the Welch's t-test for comparing the means, which does not assume equal variances.

```{r}
ggarrange(gghistogram(cc_biomass, main = "Clear Cut"),
          gghistogram(og_biomass, main = "Old Growth"))

t.test(coastal_sally_clean$weight_g ~ coastal_sally_clean$section, var.equal = FALSE)
```
The results from the test show that the mean weight of salamanders in clear cut forests is significantly higher than in old growth forests. Therefore, can reject the null hypothesis that there is no significant difference in biomass (weight) between the Coastal Giant salamanders in clear cut and old growth forest types.
...
trout_clean_correlation <- and_vertebrates |>
  filter(species == "Cutthroat trout") |>
  drop_na(length_1_mm, weight_g)

ggarrange(gghistogram(trout_clean_correlation$length_1_mm, title = "Snout to Fork Length (mm)"),
          gghistogram(trout_clean_correlation$weight_g, title = "Weight (g)"))
```
Both look skewed so they most likely do not follow a normal distribution, thus I will preform the Shapiro-Wilk normality test to check this assumption. If they do not follow a normal distribution I will then preform the Spearman rank correlation test.

```{r}
cor.test(trout_clean_correlation$length_1_mm, trout_clean_correlation$weight_g, method = "spearman")
```
With a high correlation coefficient and a low p-value, the results indicate a positive relationship between body length (snout to fork length) and body mass for cutthroat trout.
...
carapace_width <- ggplot(pie_sites, aes(x = site, y = size, fill = site)) +
  geom_boxplot() +
...
ggsave("images/carapace_width.png")
carapace_width
```
The ANOVA test for carapace width across the three selected sites (DB, GTM, PIE), reflects a significant differences in carapace width between the sites.  
...
```{r}
pie_lm_water_temp_sd <- lm(size ~ water_temp_sd, data = pie_crab)

summary(pie_lm_water_temp_sd)

water_temp_linear_regression <- ggplot(pie_crab, aes(x = water_temp_sd, y = size)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", color = "blue") +
  labs(title = "Carapace Width vs Water Temperature SD",
       x = "Water Temperature SD",
       y = "Carapace Width (mm)") +
  theme_minimal()

ggsave("images/water_temp_linear_regression.png")
water_temp_linear_regression
```
The simple linear regression showed that water temperature variation does not have a statistically significant effect on fiddler crab size. This is evident due to the low R-squared value and the insignificant p-value.
...
summary(pie_mlm)
```
The results suggest that latitude is an important factor influencing fiddler crab size, while temperature variations (both air and water) do not show strong relationships with crab size. 
